...
uint64_t k_crc32(uint8_t *arena, uint64_t size)
{
    uint32_t table[256];
...
uint64_t k_sha256(uint8_t *arena, uint64_t size)
{
...
uint64_t k_fnv64(uint8_t *arena, uint64_t size)
{
...
static void sort_longs(long *values, long low, long high)
{
    while (low < high) {
        long pivot = values[(low + high) / 2], i = low, j = high;
        while (i <= j) {
...
uint64_t k_sort(uint8_t *arena, uint64_t size)
{
    long *values = (long *)arena;
    size_t count = size;
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        state ^= state >> 17;
        state ^= state << 5;
        values[i] = (long)(state % 1000003u);
    }
    sort_longs(values, 0, (long)count - 1);
    uint64_t sum = 0;
...
uint64_t k_list(uint8_t *arena, uint64_t size)
{
    size_t count = size;
...
uint64_t k_geometry(uint8_t *arena, uint64_t size)
{
    size_t count = size;
...
uint64_t k_matrix(uint8_t *arena, uint64_t size)
{
    int n = (int)size;
...
uint64_t k_text(uint8_t *arena, uint64_t size)
{
